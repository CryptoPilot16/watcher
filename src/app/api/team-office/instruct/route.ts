...
- 40 Coders (4 per manager): c1 = tests (claude haiku), c2 = integration glue (claude haiku), c3 = fixtures/seeds (claude haiku), c4 = QA reviewer (codex /goal).
...
- You can read the codebase via your Read/Glob/Grep tools to ground your reasoning.`;
  ...
  if (meta.role === 'manager') {
    return [
      ...
      `- You lead 4 coders: c1 (tests), c2 (integration glue), c3 (fixtures/seeds), c4 (QA reviewer, autonomous /goal). Allocate to them via the autopilot's <<CODERS>>...<<END>> block — only where useful, no makework.`,
...
function engineForAxiomTopic(sessionKey: string): 'claude' | 'codex' {
  ...
  // c4 is the team's QA reviewer. Multi-step audit loop benefits from
  // /goal autonomy.
