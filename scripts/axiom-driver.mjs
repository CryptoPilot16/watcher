...
function buildManagerBrief(team, roadmapHint) {
  ...
  return [
    ...
    `Reply ≤400 chars + <<CODERS>>...<<END>> block. Allocate ONLY coders with real work. Hints: c1=tests c2=glue c3=fixtures c4=QA-reviewer. Skip a c-line if useless. Empty block = team rests.`,
    `Format:`,
    ...
    `c2: <only if useful>`,
    `c3: <only if useful>`,
    `c4: <only if useful — what to audit/fix>`,
    `<<END>>`,
  ].filter(Boolean).join('\n');
}

// Parse a manager's reply for a <<CODERS>> ... <<END>> block. Returns
// { 1, 2, 3, 4 } with whatever briefs the manager assigned, or null
// per-coder if the line was missing. Teams have 4 coders: c1=tests,
// c2=glue, c3=fixtures, c4=QA.
...
function buildCoderBrief(team, coderIndex, managerAssignedTask) {
  ...
  return [
    ...
    ``,
    `Your role hint: c${coderIndex}=${coderIndex === 1 ? 'tests' : coderIndex === 2 ? 'integration glue' : 'fixtures/seeds'}. (c4 is your team's QA reviewer — they will audit your work next round, so write code that's actually correct.)`,
    ``,
